...
export default function main(): void {
  ...
  const type = context.triggerConfig.type
  if (type !== TriggerType.CRON) throw new Error('Trigger config type is not cron')

  const cronTrigger = context.triggerConfig.getCronData()
